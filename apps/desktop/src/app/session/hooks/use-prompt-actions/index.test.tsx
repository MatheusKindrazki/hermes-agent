...
describe('usePromptActions submit / queue drain semantics', () => {
  ...
  it('a NON-queue explicit target keeps its runtime id even with no central binding recorded', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      { source_event_id: expect.stringMatching(/^[0-9a-f-]{36}$/), session_id: 'rt-tab', text: 'kickoff for the tab' },
      1_800_000
    )
  })
  ...
  it('a rejected fromQueue drain returns false (entry stays queued) and a later retry sends it', async () => {
    ...
    const first = await handle!.submitText('please send me', { fromQueue: true, sourceEventId })
    expect(first).toBe(false)

    const second = await handle!.submitText('please send me', { fromQueue: true, sourceEventId })
    expect(second).toBe(true)
    expect(
      requestGateway.mock.calls.filter(call => call[0] === 'prompt.submit').map(call => call[1]?.source_event_id)
    ).toEqual([sourceEventId, sourceEventId])
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      {
        source_event_id: expect.stringMatching(/^[0-9a-f-]{36}$/),
...
describe('usePromptActions sleep/wake session recovery', () => {
  ...
  it('recovers via session.resume when prompt.submit TIMES OUT and a stored session is selected (#55578)', async () => {
    ...
    const providedSourceId = '11111111-1111-4111-8111-111111111111'
    const ok = await handle!.submitText('message during starved loop', { sourceEventId: providedSourceId })

    expect(ok).toBe(true)
    const sourceIds = calls.filter(call => call.method === 'prompt.submit').map(call => call.params?.source_event_id)
    expect(sourceIds).toHaveLength(2)
    expect(sourceIds[0]).toMatch(/^[0-9a-f-]{36}$/)
    expect(sourceIds[1]).toBe(sourceIds[0])
    expect(sourceIds[0]).toBe(providedSourceId)
    expect(calls.map(c => c.method)).toEqual(['prompt.submit', 'session.resume', 'prompt.submit'])
    expect(calls[1]?.params).toEqual({
      session_id: STORED_SESSION_ID,
      source: 'desktop',
...
describe('usePromptActions new-chat first-send delivery (#63078)', () => {
  ...
  it('delivers the first prompt when React Router commits the created-session route late (#62990)', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      { source_event_id: expect.stringMatching(/^[0-9a-f-]{36}$/), session_id: NEW_RUNTIME_ID, text: 'hello' },
      1_800_000
    )
  })
